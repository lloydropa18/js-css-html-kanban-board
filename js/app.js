...
function addTasks() {
  // advanced: you can pass thjrough the whole task object if you wish
  tasks.forEach((task) => createTask(task.id, task.title, task.description));
}

addTasks();

// Change background color of the task
...
let elementBringDragged;
function dragStart() {
  elementBringDragged = this;
}
...
function dragDrop() {
  ...
  elementBringDragged.firstChild.style.backgroundColor = addColor(columnId);
  this.append(elementBringDragged);
}
...
function deleteTask() {
  ...
  tasks = tasks.filter((task) => {
    return task.title !== filteredTasks[0];
  });
